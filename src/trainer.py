...
from .util import get_psnr, get_mse, get_psnr_3d, get_ssim_3d, cast_to_image
...
class Trainer:
    ...
    def start(self):
        ...
        for idx_epoch in range(self.epoch_start, self.epochs): # trange(self.idx_epoch_start, self.epochs, desc='epoch'):
            ...
            pbar = tqdm(total=len(self.train_dloader) * self.train_dloader.batch_size, 
                bar_format='{desc}: {percentage:3.0f}% {n_fmt}/{total_fmt} [{elapsed}<{remaining}, {rate_fmt}]', leave=False)
            ...
            for data in self.train_dloader:
                ...
                pbar.update(self.train_dloader.batch_size)
